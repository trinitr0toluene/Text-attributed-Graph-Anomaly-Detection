...
class CLIP(nn.Module):
    def __init__(self,
        ...
        gcn = GCN(in_channels=args.gnn_input,
                  ...
                  use_bn=args.use_bn)
        self.gnn = SGFormer(args.gnn_input, args.gnn_hid, args.gnn_output, num_layers=args.trans_layers,
    ...
    def encode_image(self, x, adj, batch):
        embs = self.gnn(x,adj,batch)
        # G.ndata['h'] = node_embs
        # graph_embs = dgl.mean_nodes(G, 'h')
        return embs
    ...
    def forward(self, data, device, training=True):
        x, adj, batch = data.x.to(device), data.edge_index.to(device), data.batch.to(device)
        # root_features = data.root_features
        # root_features = root_features.reshape(root_features.size(0)//768, 768).to(device)

        pt_labels = np.load('embed_lb.npy')
        pt_labels = torch.tensor(pt_labels, dtype=torch.float).to(device)
        s_text_features = pt_labels @ self.text_projection

        s_image_features = self.encode_image(x,adj,batch)
        # s_text_features = self.encode_text(root_features)
        # s_text_features = root_features @ self.text_projection
    ...
    def inference(self, data, device):
        ...
        labels = np.load('embed_lb.npy')
        labels = torch.tensor(labels, dtype=torch.float).to(device)
        # labels = labels.reshape(labels.size(0)//768, 768).to(device)

        s_image_features = self.encode_image(x,adj,batch)
        s_text_features = labels @ self.text_projection
        ...
        pred = preds.detach().clone().requires_grad_(True)

        # labels = torch.arange(s_text_features.shape[0]).to(device)
        return pred
    
    def finetune(self, data, device, training = True):
        x, adj, batch = data.x.to(device), data.edge_index.to(device), data.batch.to(device)
        true = data.label.to(device)
        ...
        preds = torch.argmax(logits, dim=1).float()
        pred = preds.detach().clone().requires_grad_(True)
        all_loss = F.cross_entropy(pred, true)
